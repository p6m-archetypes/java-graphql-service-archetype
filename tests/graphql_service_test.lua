...
for _, v in ipairs(VARIANTS) do
  ...
  local service = prova.fixture(label .. ":service", Scope.File, function(ctx)
    ...
    return { readiness = readiness, api = api, conn = d.conn }
  end)
...
local none = prova.fixture("java-graphql[None]:project", Scope.File, function(ctx)
  return render(ctx, "None")
end)

prova.group("java-graphql[None]", { requires = { "archetect" } }, function(g)
